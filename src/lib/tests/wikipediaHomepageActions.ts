...
export async function run(page: Page, params: {}) {
    ...
    const totalArticlesLinkCount = await wikipediaHomePage.getEnglishArticleCount();
    const comparableArticlesLinkCount = 7000000
    console.log(`English articles count: ${totalArticlesLinkCount}`);
    expect(totalArticlesLinkCount).toBeLessThan(comparableArticlesLinkCount);

    /** STEP: Select the 'Small' text size option in the appearance settings */
    let originalSize = await wikipediaHomePage.getBodyFontSize();
    ...
    let largeSize = await wikipediaHomePage.getBodyFontSize();
